...
let PhoneSubscriptionService = class PhoneSubscriptionService {
    ...
    async getPhoneById(ctx, data) {
        return core_1.getEntityOrThrow(this.connection, phonesubscription_entity_1.PhoneSubscriptionEntity, data);
    }
    async addSinglePhone(ctx, data) {
        const createdVariant = this.connection.getRepository(phonesubscription_entity_1.PhoneSubscriptionEntity).create(data);
        const savedVariant = await this.connection.getRepository(phonesubscription_entity_1.PhoneSubscriptionEntity).save(createdVariant);
        return Object(savedVariant).id;
    }
    async updateSinglePhone(ctx, data) {
        const createdVariant = await this.connection.getRepository(phonesubscription_entity_1.PhoneSubscriptionEntity).update(data.id, { phone: data.phone });
        return data.id;
    }
    async addPhone(ctx, input) {
        ...
        const createdVariants = await this.connection.getRepository(phonesubscription_entity_1.PhoneSubscriptionEntity).findByIds(ids);
    ...
    async deletePhone(ctx, ids) {
        const Variants = await this.connection.getRepository(phonesubscription_entity_1.PhoneSubscriptionEntity).findByIds(ids);
        this.connection.getRepository(phonesubscription_entity_1.PhoneSubscriptionEntity).delete(ids);
        return Variants;
    }
    deleteAllPhones(ctx) {
        this.connection.getRepository(phonesubscription_entity_1.PhoneSubscriptionEntity).clear();
        return true;
    }
};
